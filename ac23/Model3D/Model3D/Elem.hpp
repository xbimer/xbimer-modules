...
class MODEL3D_DLL_EXPORT Elem : public GS::SharedObject
{
	...
	MODEL3D_DLL_EXPORT friend GSErrCode Store (GS::ObjectState& os, const Elem& elem);
	MODEL3D_DLL_EXPORT friend GSErrCode Restore (const GS::ObjectState& os, Elem& elem);
private:
	struct StatusType {
...
public:
	...
	enum ComparisonFlags {
		WithoutGenId		= 0x01,
		WithoutElemGuid		= 0x02
	};

	bool		Equals (const Modeler::Elem& other, const UInt32 flags = WithoutGenId) const;
	bool		EqualsExactly (const Modeler::Elem& other, const UInt32 flags = WithoutGenId) const;
	...
	const TRANMAT*	GetConstTrafoPtr () const;
	...
	const ConstBaseElemPtr&				GetConstBaseElemPtr () const;
	...
	virtual const F_Box3D&	GetBounds () const;
...
inline const TRANMAT*	Modeler::Elem::GetConstTrafoPtr () const { return &trafo; }
...
inline const ConstBaseElemPtr& Modeler::Elem::GetConstBaseElemPtr () const { return baseElem; }
