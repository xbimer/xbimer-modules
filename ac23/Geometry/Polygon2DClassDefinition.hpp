...
namespace Geometry {
	...
	public:
		...
		{
			os.Add ("polygon", *(polygon.polygon));
			return NoError;
		}
		...
		friend GSErrCode	Restore (const GS::ObjectState& os, Geometry::CustomPolygon2D<VertexUserData, EdgeUserData, ContourUserData, PolygonUserData>& polygon)
		{
			...
			polygon.polygon = new Geometry::CorePolygon2D<VertexUserData, EdgeUserData, ContourUserData, PolygonUserData> (flags);
			os.Get ("polygon", *(polygon.polygon));
			return NoError;
		}
		//******************************* Calculate *******************************
	...
	public:
		...
		CustomMultiPolygon2D (GS::ByRefSelector, GS::InitializerRefList<PolyType> items);
