...
template<class DInt, class Int, class PolygonSelector>
inline Point<DInt,Int> PGBoolTmp<DInt,Int,PolygonSelector>::CoordToPoint (const Point2D& c)
{
	Point<DInt,Int> p;
...
template<class DInt, class Int, class PolygonSelector>
inline Point2D PGBoolTmp<DInt,Int,PolygonSelector>::PointToCoord (const Point<DInt,Int>& p)
{
	Point2D c;
...
template<class DInt, class Int, class PolygonSelector>
typename PGBoolTmp<DInt,Int,PolygonSelector>::Node* PGBoolTmp<DInt,Int,PolygonSelector>::InsertNewNode (const Point2D& c, WhichPoly whichPoly, Int32 origVertex)
{
...
template<class DInt, class Int, class PolygonSelector>
typename PGBoolTmp<DInt,Int,PolygonSelector>::Node* PGBoolTmp<DInt,Int,PolygonSelector>::InsertNewLine (Node* begNode, const Point2D& endCo)
{
...
template<class DInt, class Int, class PolygonSelector>
typename PGBoolTmp<DInt,Int,PolygonSelector>::Node* PGBoolTmp<DInt,Int,PolygonSelector>::InsertNewArc (Node* begNode, const Point2D& endCo, const Point2D& center,
																	   bool revOriented)
{
	Node* endNode = NewNode ();
	try { allNodes.Push (endNode); }
...
template<class DInt, class Int, class PolygonSelector>
void PGBoolTmp<DInt,Int,PolygonSelector>::CloseContourWithArc (Node* begNode, Node* endNode, const Point2D& center, double r, bool revOriented)
{
	if (begNode == endNode)
		return;
...
template<class DInt, class Int, class PolygonSelector>
void PGBoolTmp<DInt,Int,PolygonSelector>::SegmentArcToLines (Node* beg, Node* end, const Point2D& center, bool revOriented)
{
	Point2D b = PointToCoord (beg->pos);
	Point2D e = PointToCoord (end->pos);
	Geometry::Vector2d b2 = b - center;
	Geometry::Vector2d e2 = e - center;

	double alpha = atan2 (b2 ^ e2, Geometry::MulsVectors (b2, e2));
	if (alpha < 0)
		alpha += 2 * PI;
	if (revOriented)
		alpha = 2 * PI - alpha;
	...
	for (Int32 i = 1; i < nSegments; i++) {
		...
		Point2D s = center + (MulVector (x, cos (beta * i)) + MulVector (y, sin (beta * i)));
...
void PGBoolTmp<DInt,Int,PolygonSelector>::ReturnContourProc (const Contour& contour, PGPOLYEXT* createdPoly)
{
	...
	do {
		...
		Point2D end = PointToCoord (left ? link->end->pos : link->beg->pos);
		...
		if (!link->isLine) {
			...
			edge.radius = (edge.begPt - edge.center).GetLength ();
...
void PGBoolTmp<DInt,Int,PolygonSelector>::ReturnContourData (const Contour& contour, Polygon2DData* poly)
{
	...
	do {
		...
		if (!link->isLine) {
			...
			Point2D center = PointToCoord (arc->center);
			Point2D end = PointToCoord (left ? arc->end->pos : arc->beg->pos);
			Geometry::Vector2d a = UnitVector ((*poly->vertices)[poly->nVertices] - center);
			Geometry::Vector2d b = UnitVector (end - center);			
			if (left != arc->positiv) {
				GS::Swap (a, b);
			}
			(*poly->arcs)[poly->nArcs].arcAngle = atan2 (a ^ b, Geometry::MulsVectors (a, b));
	...
	if (newPolygons->newPolygon == nullptr) {
		...
		if (noChange && origPolys.GetSize () < 2 && mainContours.GetSize () == 1) {
			...
			for (Int32 i = 0; i < Int32 (mainContours.GetSize ()); i++) {
				...
				try {
					...
					BMXhSetSize ((GSHandle) poly.vertices, sizeof (Point2D) * (poly.nVertices + 1));
		...
		if (noChangeOneContour) {
			...
			for (Int32 i = 0; i < Int32 (mainContours.GetSize ()); i++) {
				...
				Point2D cMin = PointToCoord (pMin);
				Point2D cMax = PointToCoord (pMax);
...
void PGBoolTmp<DInt,Int,PolygonSelector>::PrintContour (const Contour& contour, GS::OChannel& outChannel)
{
	Link* link = contour.startLink;
	do {
		Point2D beg = PointToCoord (link->beg->pos);
		outChannel << beg.x << ",  " << beg.y << ",   1, \n";

		if (!link->isLine) {
			Arc* arc = (Arc*) link;
			Point2D center = PointToCoord (arc->center);
			Point2D end = PointToCoord (arc->end->pos);
