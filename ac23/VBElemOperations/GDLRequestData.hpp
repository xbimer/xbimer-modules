...
public:
	...
	GDLRequestData (void) :	GDLGlobalCalcEnvir (),
		appVersion (0),
		appKeySerialNum (0),
	...
	GDLRequestData (const GS::UniString&								appNameIn,
					...
					ODB::ConstRef<VBD::FloorPlanCutLevels>				floorPlanCutLevelsIn,
		...
		fontNamesListPtr (fontNamesListPtrIn)
	{
		...
		if (floorPlanCutLevelsIn != nullptr)
			SetFloorPlanCutLeves (floorPlanCutLevelsIn);
	}
	
	virtual ~GDLRequestData ();

	GDLRequestData&	operator= (const GDLRequestData& source);
};
