...
namespace Imp {
template <class Type>
constexpr bool	HasInternalReadMethod (decltype (GSErrCode { std::declval<Type> ().Read (std::declval<IChannel&> ()) })*) { return true; }

template <class Type>
constexpr bool	HasInternalReadMethod (...) { return false; }

template <class Type>
constexpr bool	HasInternalWriteMethod (decltype (GSErrCode { std::declval<const Type> ().Write (std::declval<OChannel&> ()) })*) { return true; }

template <class Type>
constexpr bool	HasInternalWriteMethod (...) { return false; }

template <class Type>
constexpr bool     HasInternalRead = Imp::HasInternalReadMethod<Type> (nullptr);

template <class Type>
constexpr bool     HasInternalWrite = Imp::HasInternalWriteMethod<Type> (nullptr);
} // namespace Imp
...
template <class Type>
using HasNoObjectType = EnableIf<!IsConvertible<Type*, Object*>, bool>;
...
public:
   ...
   template <class Type, HasReadMethod<Type> = true, HasNoObjectType<Type> = true>
   GSErrCode   Read (Type& arg) { return arg.Read (*this); }
...
public:
	...
	template <class Type, HasWriteMethod<Type> = true, HasNoObjectType<Type> = true>
	GSErrCode	Write (const Type& arg) { return arg.Write (*this); }
    ...
    template <class Type, HasWriteMethod<Type> = true, HasNoObjectType<Type> = true>
    OChannel&	operator<< (const Type& arg) { arg.Write (*this); return *this; }
