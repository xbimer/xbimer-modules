...
namespace Imp {
template <class T>
constexpr bool	HasInternalStoreMethod (decltype (ErrorStatus { std::declval<const T> ().Store (std::declval<ObjectState&> ()) })*) { return true; }

template <class T>
constexpr bool	HasInternalStoreMethod (...) { return false; }

template <class T>
constexpr bool	HasInternalRestoreMethod (decltype (ErrorStatus { std::declval<T> ().Restore (std::declval<const ObjectState&> ()) })*) { return true; }

template <class T>
constexpr bool	HasInternalRestoreMethod (...) { return false; }

template <class T>
constexpr bool     HasInternalStore = Imp::HasInternalStoreMethod<T> (nullptr);

template <class T>
constexpr bool     HasInternalRestore = Imp::HasInternalRestoreMethod<T> (nullptr);
} // namespace Imp

template <class T>
using HasStoreMethod = EnableIf<Imp::HasInternalStore<T>, bool>; 

template <class T>
using HasRestoreMethod = EnableIf<Imp::HasInternalRestore<T>, bool>; 
...
private:
	...
	template <class T>
	GSErrCode	AddValue (Array<Value*>& list, const T& value, NotEnumerable<T> = true, Convertible<T> = true);

	template <class T>
	GSErrCode	AddValue (Array<Value*>& list, const T& value, NotEnumerable<T> = true, NotConvertible<T> = true);
	...
	template <class T>
	GSErrCode		GetValue (const Array<Value*>& list, UIndex idx, T& outValue, NotEnumerable<T> = true, Convertible<T> = true) const;

	template <class T>
	GSErrCode		GetValue (const Array<Value*>& list, UIndex idx, T& outValue, NotEnumerable<T> = true, NotConvertible<T> = true) const;
...
public:
	...
	public:
		Schema () = default;
		Schema (const Schema& source) = default;
...
template <class T, HasStoreMethod<T> = true>
inline auto    Store (ObjectState& os, const T& value)
{
    return value.Store (os);
}

template <class T, HasRestoreMethod<T> = true>
inline auto    Restore (const ObjectState& os, T& value)
{
...
template <class T>
GSErrCode	ObjectState::AddValue (Array<Value*>& list, const T& value, NotEnumerable<T>, Convertible<T>)
{
	typename ObjectStateTypeConversion<T>::ConvertedType convertedValue;
	GSErrCode err = ObjectStateTypeConversion<T>::Convert (value, convertedValue);
...
template <class T>
GSErrCode	ObjectState::AddValue (Array<Value*>& list, const T& value, NotEnumerable<T>, NotConvertible<T>)
{
	Owner<ObjectState> os (new ObjectState);
...
template <class T>
GSErrCode	ObjectState::GetValue (const Array<Value*>& list, UIndex idx, T& outValue, NotEnumerable<T>, Convertible<T>) const
{
	typename ObjectStateTypeConversion<T>::ConvertedType convertedValue;
	...
	return ObjectStateTypeConversion<T>::Convert (convertedValue, outValue);
}


template <class T>
GSErrCode	ObjectState::GetValue (const Array<Value*>& list, UIndex idx, T& outValue, NotEnumerable<T>, NotConvertible<T>) const
{
	...
	if (os == nullptr)
		...
		return ToErrorCode (Restore (*os, outValue));
}
...
{
	const Array<Value*>* array = GetValueArray (reinterpret_cast<const ListValue*> (list[idx]));
